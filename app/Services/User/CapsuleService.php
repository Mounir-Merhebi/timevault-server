...
class CapsuleService
{
    static function createCapsule($data)
    {
        ...
        $validatedData['gps_latitude'] = $location ? $location->latitude : null;
        $validatedData['gps_longitude'] = $location ? $location->longitude : null;
        ...
        if (!empty($validatedData['cover_image_url']) && str_starts_with($validatedData['cover_image_url'], 'data:image/')) {
            $validatedData['cover_image_url'] = self::saveBase64Image($validatedData['cover_image_url']);
        }

        if (!empty($validatedData['media_image_url']) && str_starts_with($validatedData['media_image_url'], 'data:image/')) {
            $validatedData['media_image_url'] = self::saveBase64Image($validatedData['media_image_url']);
        }


        if (!empty($validatedData['audio_file_url']) && str_starts_with($validatedData['audio_file_url'], 'data:audio/')) {
        ...
        if (request()->hasFile('text_note')) { 
            ...
            file_put_contents($filePath, file_get_contents($textNoteFile->getRealPath()));
